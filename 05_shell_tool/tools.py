"""
Tool registry and built-in tools for Step 05.

This step adds shell execution with safety controls:
- safe mode flag (default ON)
- per-command confirmation
- allow-once / always-allow / deny choices
- persistent allowlist/denylist
"""
...
def _shell_policy_file() -> Path:
    return _workspace_root() / "05_shell_tool" / ".shell_policy.json"
...
def _load_shell_policy_if_needed() -> None:
    ...
    try:
        ...
        # Ignore malformed policy and continue with empty policy.
        pass
    finally:
        _SHELL_POLICY_LOADED = True
...
def _resolve_in_workspace(path_str: str) -> Path:
    ...
    # Friendly alias for users used to container/workspace paths.
...
def _run_shell_command(command: str, timeout_sec: int) -> str:
...
@tool(
    name="run_shell",
    description="Execute a shell command. In safe mode, each new command requires explicit user approval.",
    parameters={
        "type": "object",
        "properties": {
...
def run_shell(command: str, timeout_sec: int = 30) -> str:
    _load_shell_policy_if_needed()
    cmd = command.strip()
    if not cmd:
    ...
    if _SHELL_SAFE_MODE:
        ...
        if cmd in _SHELL_ALLOWLIST:
            return _run_shell_command(cmd, timeout_sec)
