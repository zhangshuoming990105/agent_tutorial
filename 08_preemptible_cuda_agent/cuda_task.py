...
_TASK_CONTEXT_FILES = ("task.md", "TASK.md")
_GLOBAL_TASK_TEMPLATE_FILES = (
...
def find_task_context_file(task_dir: str | Path) -> Path | None:
    """Return the first existing task context markdown file under task_dir."""
    task_dir = Path(task_dir).resolve()
    for name in _TASK_CONTEXT_FILES:
...
def load_task_prompt(task_dir: str | Path) -> tuple[str, Path | None]:
    """Load task.md/TASK.md and convert it into a system prompt fragment.

    Returns:
      (prompt_text, source_file_path_or_none)
    """
    src = find_task_context_file(task_dir)
    if src is None:
        # Fallback to a global template for task families.
        for rel in _GLOBAL_TASK_TEMPLATE_FILES:
            candidate = (_SELF_DIR / rel).resolve()
            if candidate.is_file():
                src = candidate
                break
        if src is None:
    ...
    lines = [
        ...
        "Follow it unless it conflicts with higher-level safety/policy constraints.",
        "",
        f"Source: {src}",
        "",
        text,
