"""
Tool registry and built-in tools for Step 06.
...
def _shell_policy_file() -> Path:
    return _workspace_root() / "06_error_recovery" / ".shell_policy.json"
...
def run_shell(command: str, cwd: str | None = None, timeout_sec: int = 30) -> str:
    _load_shell_policy_if_needed()
    cmd = command.strip()
    if not cmd:
